...
ads_journals.py
===============
Per-paper published-journal enrichment from the NASA ADS API.

Reads arXiv ids + titles from datasets/arxiv_papers.csv, resolves each paper
in ADS (arXiv identifier first, title search as fallback), and records the
ADS `pub` (publication / journal) field.

Output: datasets/ads_journals.csv with columns
  arxiv_id, title, ads_bibcode, ads_journal, status

Resume-safe (Ctrl+C safe): finished ids are tracked in
datasets/ads_journals_state.json and skipped on the next run.

Access / compliance: same as ads_citations.py — ADS API use requires a
personal token (ADS_API_KEY env / .env), ~5000 requests/day, with a polite
pause between calls and a 60 s backoff on HTTP 429.

Usage:
  export ADS_API_KEY="your_token_here"
  python ads_journals.py                 # full run, resumable
  python ads_journals.py --limit 10      # quick test on 10 papers
  python ads_journals.py --batch-size 5  # batch arXiv-id resolution
  python ads_journals.py --restart       # ignore saved progress
"""
...
OUTPUT_CSV = os.path.join(HERE, "datasets", "ads_journals.csv")
STATE_PATH = os.path.join(HERE, "datasets", "ads_journals_state.json")
...
OUTPUT_COLUMNS = ["arxiv_id", "title", "ads_bibcode", "ads_journal", "status"]
...
def ads_request(params, timeout=20):
    while True:
        try:
            r = requests.get(ADS_URL, headers={"Authorization": f"Bearer {ADS_API_KEY}"},
...
def resolve_batch(batch_rows):
    ...
    data = ads_request({"q": q, "fl": "bibcode,pub,identifier", "rows": len(ids) * 2})
    if not data:
        return mapping
...
def process_paper(arxiv_id, title, batch_doc):
    ...
    if doc is None:
        ...
        if clean:
            data = ads_request({"q": f'title:"{clean}"', "fl": "bibcode,pub", "rows": 1})
            if data and data.get("response", {}).get("docs"):
                doc = data["response"]["docs"][0]
    if not doc:
        return {"arxiv_id": arxiv_id, "title": title, "ads_bibcode": "",
                "ads_journal": "", "status": "not_found"}
    return {"arxiv_id": arxiv_id, "title": title,
            "ads_bibcode": doc.get("bibcode", ""),
            "ads_journal": (doc.get("pub", "") or "").strip(),
            "status": "success"}
...
def main():
    ...
    try:
        for i in range(0, len(todo), args.batch_size):
            ...
            rows = [process_paper(aid, title, mapping.get(aid)) for aid, title in batch]
            append_rows(rows)
            for r in rows:
